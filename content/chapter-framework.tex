...
During development of the models, we realized that there are highly divergent understandings regarding the batch construction for sequential data. We believe that these different understandings strongly impact the reproducibility of any prediction approach.
Furthermore, we needed to automate the training of the networks because we did so on five datasets.
We present a solution to both challenges here with a model training framework. We share it because we believe that it enables researchers to compare the different understandings and easily try out new model architectures. In doing so, we hope to contribute to Improvement Area 1 and 2 in \autoref{sec:intro:motivation}.
...
This concludes the presentation of the batching problem with sequential data,
and how the training framework helps to address it.
In the next chapter we explain which datasets we use the framework with,
